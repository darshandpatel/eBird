...
class DataExploration:
    ...
    @staticmethod
    def add_protocol_list(x, header_dict):
        protocol = x[header_dict["COUNT_TYPE"]]
        if protocol == -1:
            return x
        #Append 20 columns
    ...
    @staticmethod
    def add_time_slot(x, header_dict):
        time = DataExploration.get_number(x[header_dict["TIME"]])
    ...
    @staticmethod
    def add_elev_avg(x, header_dict):
        elev_gt_colID = header_dict["ELEV_GT"]
        elev_ned_colID = header_dict["ELEV_NED"]
    ...
    @staticmethod
    def add_xyz(lx, header_dict):
        long_colID = header_dict["LONGITUDE"]
        latt_colID = header_dict["LATITUDE"]
    ...
    @staticmethod
    def add_columns(x, header_dict):
        px = DataExploration.add_protocol_list(x, header_dict)
        ptx = DataExploration.add_time_slot(px, header_dict)
        ptex = DataExploration.add_elev_avg(ptx, header_dict)
        ptelx = DataExploration.add_xyz(ptex, header_dict)
        return ptelx

    #Missing CAUS features are replaced from extended covariates.
    @staticmethod
    def replace_caus(x, header_dict):
        prec_cid = header_dict["CAUS_PREC"]
        snow_cid = header_dict["CAUS_SNOW"]
        tavg_cid = header_dict["CAUS_TEMP_AVG"]
        tmin_cid = header_dict["CAUS_TEMP_MIN"]
        tmax_cid = header_dict["CAUS_TEMP_MAX"]
        month = int(max(1.0,DataExploration.get_number(header_dict["MONTH"])))
        ...
        precmm_cid = header_dict["CAUS_PREC"+mm]
        try:
            snowmm_cid = header_dict["CAUS_SNOW"+mm]
            snowmm = x[snowmm_cid]
        except KeyError:
            snowmm = 0
        tavgmm_cid = header_dict["CAUS_TEMP_AVG"+mm]
        tminmm_cid = header_dict["CAUS_TEMP_MIN"+mm]
        tmaxmm_cid = header_dict["CAUS_TEMP_MAX"+mm]
        #replace if missing
        if x[prec_cid] == "?":
    ...
    @staticmethod
    def replace_columns(ls, header_dict):
        cx = DataExploration.replace_caus(ls, header_dict)
        return cx

    #Drop columns present in Drop_list and multiple_Drop_list.
    @staticmethod
    def drop_columns(ls, header_dict):
    ...
    @staticmethod
    def filter_value_by_checklist_header(value, header_dict):
        #filter columns where PRIMIARY_CHECKLIST_FLAG is set to false
        #Keep it if the target value is 1 or x.
        lx = value.split(",")
        if lx[header_dict["PRIMARY_CHECKLIST_FLAG"]] == "1" or \
                (lx[header_dict["Agelaius_phoeniceus"]] != '0' and
                         lx[header_dict["Agelaius_phoeniceus"]] != '?'):
            return True
        else:
            return False
    ...
    @staticmethod
    def custom_function(ls, is_val, is_test, header_dict, birds_columns, drop_index):
        #One-Hot encoding
        a_ls = DataExploration.add_columns(ls, header_dict)
        #Manipulate values
        rca_ls = DataExploration.replace_columns(a_ls, header_dict)
        #Convert into numeric and target column to binary
        n_ls = HandleMissing.convert_into_numeric_value(rca_ls,
                                                        dict=header_dict,
                                                        birds_index=birds_columns,
                                                        drop_index=drop_index)
        tn_ls = None
        #for testing
        if (not is_test):
            tn_ls = HandleMissing.convert_target_column_into_numeric(n_ls,
                                                                     target_index=header_dict["Agelaius_phoeniceus"], )
        else:
            tn_ls = n_ls
        #drop columns
        drca_ls = DataExploration.drop_columns(tn_ls, header_dict)
        if is_val:
    ...
    @staticmethod
    def cal_birds_column_ids(header_dict):
        bird_index = []
        ...
        return np.array(bird_index)

    #populate teh list of columns that are dropped.
    @staticmethod
    def cal_drop_column_ids(header_dict):

        drop_index = []
        for col in DataExploration.drop_list:
            try:
                l = len(col)
                drop_index.extend(header_dict[col])
            except TypeError:
                drop_index.append(header_dict[col])

        for col in DataExploration.drop_multiples_list:
            for key in header_dict:
                if key.startswith(col):
                    drop_index.append(header_dict[key])
        #DataExploration.drop_column_ids = drop_index.sort()
    ...
    def perform_distributed_ml(self, train_rdd, model_path, header_dict, birds_columns, drop_index):

        # Apply data cleaning and sampling operation
        processed_train_rdd = (train_rdd.filter(lambda x: DataExploration.filter_value_by_checklist_header(x, header_dict)). \
                               map(lambda x: DataExploration.swap_target(x, header_dict["Agelaius_phoeniceus"])). \
                               filter(lambda x: ModelTraining.handle_class_imbalance(x)). \
                               map(lambda x: DataExploration.custom_function(x, False, False, header_dict,
                                                                             birds_columns, drop_index)))
...
if __name__ == "__main__":
    ...
    #header_dict = DataExploration.create_header_dict(first_row)
    ...
    dataExploration.perform_distributed_ml(train_rdd, model_path, header_dict, birds_columns, drop_index)
    dataExploration.sc.stop()
