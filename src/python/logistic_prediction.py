...
class DataExploration:
    header_dict = {}
    drop_list = ["SAMPLING_EVENT_ID", "LOC_ID", "YEAR", "DAY", "COUNTRY", "STATE_PROVINCE", "COUNTY", "COUNT_TYPE",
                 "OBSERVER_ID",
    ...
    def catagories(x):
        ...
        list.append(pair)
        pair = (
        (DataExploration.get_col_id("OMERNIK_L3_ECOREGION"), x[DataExploration.get_col_id("OMERNIK_L3_ECOREGION")]), 1)
        list.append(pair)
    ...
    def find_catagories(rdd):
        print rdd.map(lambda x: x.split(",")).flatMap(
            lambda x: DataExploration.catagories(x)).groupByKey().keys().collect()
    ...
    def test_prediction_through_models(self, test_data_set, model_path, prediction_path):
        ...
        #print "model.numTrees() : ", model.numTrees()
...
if __name__ == "__main__":
    ...
    args = sys.argv
    test_path = args[1]  # "/Users/Darshan/Documents/MapReduce/FinalProject/LabeledSample"
    ...
    test_data_set = data_exploration.read_sample_training(test_path).persist()

    #DataExploration.create_header(test_data_set.first().split(','))
    ...
    data_exploration.test_prediction_through_models(test_data_set, model_path, prediction_path)